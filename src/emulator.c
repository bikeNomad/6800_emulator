...
void __time_critical_func(bus_read_block_with_eclock)(uint16_t address, uint16_t length, uint8_t *buffer) {
    if (!pause_emulator()) {
        return;
    }
...
void __time_critical_func(bus_write_block_with_eclock)(uint16_t address, const uint8_t *buffer, uint16_t length) {
    if (!pause_emulator()) {
        return;
    }
