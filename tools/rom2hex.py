...
def main():
    if len(sys.argv) < 3:
        print("Usage: rom2hex.py <start_address_hex> <rom_file1> [rom_file2] [...]", file=sys.stderr)
    ...
    print(f"# Total: {len(binary_data)} bytes at address ${start_address:04X}", file=sys.stderr)
    print(f"# Address range: ${start_address:04X}-${start_address + len(binary_data) - 1:04X}", file=sys.stderr)
